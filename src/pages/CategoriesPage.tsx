    ...
    const fetchCounts = async () => {
      ...
      await Promise.all(
        CATEGORIES.map(async (cat) => {
          try {
            const count = await blink.db.xelayQuestions.count({
              where: { category: cat },
            })
            countMap[cat] = Number(count) || 0
          } catch {
            countMap[cat] = 0
          }
        })
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="mb-10">
          <h1 className="text-4xl font-bold tracking-tight text-foreground mb-2">Categories</h1>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    {meta?.icon && <span className="text-xl">{meta.icon}</span>}
                    <span className="text-base font-bold text-foreground leading-tight">{cat}</span>
                  </div>
